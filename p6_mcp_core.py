...
@mcp.tool(
    ...
    description=(
        ...
        "actions: schedule | summarize | apply_actuals | status | wait | list "
        "| cancel | purge | preflight | service_health | job_data. Also "
        "level | update_baseline, but P6 Professional's job dispatcher cannot "
        "run those two (measured: 'Invalid Job type') -- they return an "
        "explanatory error; leveling and baseline update are GUI-only, use "
        "p6_baseline for baseline copies.\n"
...
@mcp.tool(
    name="p6_baseline",
    description=(
        "P6 baselines: list, create, assign, delete -- plus 'revision', an "
        "in-database project copy.\n"
        "actions: list | create | assign | delete | revision\n"
        "params: proj_id, baseline_proj_id, base_type (default 'Initial "
        "Plan') or base_type_id, baseline_name, assign (default true), "
        "dry_run, confirm (required to write); delete of a REAL project "
        "(e.g. a revision copy) additionally requires delete_project=true "
        "and expected_short_name matching the project's short name, and is "
        "refused while live baselines still point at it.\n"
        "'revision' uses the baseline copy machinery but leaves the copy as "
        "a real project (project_flag='Y', visible in EPS) -- the safe way "
        "to revise a Cyrillic programme, since a CLI import round-trip "
        "destroys Cyrillic text.\n"
